...
| 日期 | 主题 | 分类 | 标签 | 一句话 | 出处 |
|---|---|---|---|---|---|
| 2026-09-24 | [--stream-interval 为什么在长输出场景有提升](inference-systems/sglang-stream-interval/index.html) | inference-systems | SGLang · 流式输出 · SSE · 调度 · O(T²) · 推理服务 | --stream-interval 在源码里只有一个判定点（output_streamer.accept 的 len(output_ids) % N == 1），但真正被它砍掉的是默认非增量流式路径里的 O(T²)：每个 SSE 事件都要重建累计文本、并携带完整 text + output_ids 向下游搬运；实测每翻倍输出长度开销涨 3.9 倍，间隔 2 恒定砍半 —— 所以只有长输出才看得见。 | — |
| 2026-09-21 | [MoE 的通信：MegaMoE 内核、A2A 开关，与各后端的真实差别](inference-systems/moe-a2a-megamoe/index.html) | inference-systems | SGLang · DeepGEMM · MegaMoE · MoE · EP / A2A · DeepEP · FlashInfer · 对称内存 · 源码走读 | 对着 SGLang 与 DeepGEMM 源码讲三件事：MegaMoE 把一个 MoE 层的 dispatch、两次分组 GEMM、激活、combine 归约压进同一个全网格持久内核（拉取式 dispatch + L1 输出直落 L2 池 + 结果直推源 rank）；设与不设 --moe-a2a-backend 之间，换的是「层尾那次稠密 all-reduce」与「dispatch/combine」的取舍，并顺带改写 ep=tp、共享专家融合、CUDA graph 等七项参数；而 runner 后端与 A2A 后端是两层正交的东西，靠一张按 DispatchOutputFormat 注册的表决定谁能配谁。 | — |
